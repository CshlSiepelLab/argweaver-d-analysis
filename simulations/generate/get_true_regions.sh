...
## Get true introgressed regions for set of simulations
model=$1
## get div field. model is in form mod150_div1000, for example
sDiv=`echo $model | awk -F '_' '{print $2}' | sed 's/div//g'`
sDiv=$(($sDiv*1000))
startrep=$2
endrep=$3
if [[ -z $startrep ]]; then
